...
as it can help in marking the papers and generating a feedback.
...
PDFs are generated using the FPDF framework. 
...
to wait for long time while time-consuming operations are made. 
Bacause of this, the solution runs on mainly PHP and cookies to
...
The text that is auto-generated when creating a final raport (the explanation at the beggining of the report) can be found
in the ```text``` folder. Each section is grouped in a file which holds the corresponding name.
...
2. The database schema should be uploaded and executed. 
   Usually, this can be fone from the PHPmyAdmin interface or any Database interface which allow operations to be done
   on the database schema. The user should also have the priviliges to create and edit the database. 
   The database file is called "database.sql"
3. Move the files to a remove server using either the ZIP archive being extracted on the remote location 
   or by using a FTP client such as FileZilla to move the files to the remote location.
4. The database connection details have to be edited in the file Database.php from the Models folder. 
   ...
   (which we do not reccommend) or you can use any Base64 encoding / decoding website to encode your username and password an use that information in Database.php.
